...
vidBase = str(vid)	# Convert video file name to string for string manipulations
idxFrame = 0	# Number of Frames extracted

# Split Up video file name to extract the system time that marks when the video file began recording
_,_,date,time,tmpSysTime = vidBase.split('_')	# Split filename into relevent parts
tmpSysTime,_ = tmpSysTime.split('.') # Remove file extension from string
sysTime = int(tmpSysTime) # Convert parsed system time to numerical variable for later
...
with open(csvFile, "w") as output:
	...
	tmpData = [width, height, fps, numFrames, sysTime]
	...
	writer.writerow(["Image Name", "Time of Frame in Video (msec)", "Time of Frame in System (msec)", "Frame Index"])	# Header line
	...
	# For debugging
	print('Width, Height, FPS, # of Frames, System Time @ Start of Video (msec): %d,	%d,	%d,	%d,	%d' % (width, height, fps, numFrames, sysTime))
